...
engines is actually broadcast so any engine may pick the information it
wants.
...
\paragraph{Inductive Validity Cores (IVC).} For a proved property, an
...
(AADL) and to a plugin for the OSATE AADL Integrated Development
Environment. The \agree annex annotates the AADL model with formal requirements, and the plugin reasons about these requirements. The purpose of \agree is to
...
compositional reasoning over software. \simpal features a specification
...
which is passed to the \jkind to perform assume-guarantee reasoning, reachability, and viability
analyses. The feedback from these analyses is used to refine
the program to ensure that the software functions as intended.
...
\paragraph{State Machines.} A flight control group at Rockwell
...
test case. The tool presents all this information graphically to its users.
